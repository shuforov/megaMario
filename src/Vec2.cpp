...
Vec2 operator*(float val, Vec2 lhs) { return {lhs.x * val, lhs.y * val}; }
...
float length(Vec2 a) { return a.length(); }

float length_squared(Vec2 a) { return a.length_squared(); }
...
float dot(Vec2 lhs, Vec2 rhs) { return lhs.x * rhs.x + lhs.y * rhs.y; }

float distance(point2 lhs, point2 rhs) { return (lhs - rhs).length(); }
